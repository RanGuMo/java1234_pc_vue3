<template>
  <el-card>
    ...
    <el-table :data="tableData" stripe style="width: 100%">
      ...
      <!-- 这里有了 :formatter ,最终是以:formatter的返回值显示，所以 prop="type" 可有可无，
        甚至 prop的属性值叫什么名字都无所谓 type11111 都行，随便。不需要这个prop="type" 也行-->
...
const handleImageDialogValue=(row)=>{
  imageDialogVisible.value = true;//显示弹窗
  imageDialogValue.value = JSON.parse(JSON.stringify(row));//将这一行的数据，进行深拷贝后，赋值给imageDialogValue
}
