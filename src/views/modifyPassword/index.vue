  ...
  <el-card>
    ...
    >
      ...
      <el-form-item label="确认新密码" prop="newPassword2">
        <el-input v-model="form.newPassword2" type="password" show-password clearable/>
      </el-form-item>
    ...
    formRef.value.validate(async (valid) => {
        if (valid) {
            if (form.value.oldPassword !== form.value.password) {
                ...
                try {
                    let res = await axiosUtil.post('/admin/modifyPassword', form.value);
                    if (res.data.code === 0) {
